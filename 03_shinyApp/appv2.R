...
server <- function(input, output,session) {
    ...
    # Create an empty dataframe to hold stock prices 
    # stock_df <- data.frame(stock_symbol <- NA,weight <- NA)
    ...
    stock_prices <- reactive({
        get_stock_data_function(stockSymbols = stock_symbols(), startDate = input$input_start_date, endDate = input$input_end_date)
    })
    
    # Tidy Stock Prices & Calculate Log Returns ----
    returns_tbl <- reactive({
        stock_data_tidy_function(stock_prices(),timePeriod = "monthly")
    })
    
    output$prices2 <- renderTable(returns_tbl())
